...
import { withLoading } from '@shared/operators/loading-operator';

@Component({
  selector: 'mfe-shell-profile',
  template: `
  @if(isLoading){
    <shell-section ngxClass="relative min-h-screen flex items-center justify-center">
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.route.paramMap.subscribe((params) => {
      ...
      this.accountTypeService.checkAccountType(username)
        .pipe(
          withLoading((state) => this.isLoading = state)
          )
        .subscribe({
          next: ({ accountType }) => {
